...
<template>
  <header :class="alpine.header?.position || 'left'" class="relative grid grid-cols-12 items-center gap-4 py-16">
    <div class="menu absolute flex hover:text-purple-500 sm:hidden">
    ...
    <div
      class="overlay sm:hidden absolute top-24 bg-gray-100 dark:bg-gray-900 border-2 border-solid border-gray-200 dark:border-gray-800 rounded-md p-6 text-lg leading-8 font-medium transition-all backdrop-blur-md will-change-auto"
      :class="[show && 'show']"
    >
      <MainNav @link-click="show = !show" />
    </div>

    <div class="logo text-3xl h-12 font-extrabold flex col-span-12 sm:col-span-4 sm:col-start-auto ">
      <NuxtLink v-if="alpine.header?.logo" to="/">
        <h1 class="hover:text-purple-200">
          Rohit Prakash
        </h1>
      </NuxtLink>
    </div>

    <div class="main-nav hidden sm:flex sm:col-span-8 sm:text-xl sm:leading-6 sm:font-medium">
      <MainNav />
    </div>
  </header>
</template>
